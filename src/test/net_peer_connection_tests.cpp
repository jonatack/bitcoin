...
BOOST_AUTO_TEST_CASE(test_addnode_getaddednodeinfo_and_connection_detection)
{
    ...
    // Call ConnectNode (which is also called by RPC addnode "localhost:port" onetry) for a
    // localhost address that resolves to multiple IPs, including that of the connected peer.
    // FIXME: The new peer should consistently not be connected due to the check in ConnectNode().
    for (int i = 0; i < 10; ++i) {

...
/** Create a connected peer. If the optional `address` param isn't passed, a random address is created. */
...
BOOST_AUTO_TEST_CASE(test_addnode_getaddednodeinfo_and_connection_detection)
{
    ...
    // Run the equivalent of RPC addnode "localhost:port" onetry for 2 IPs that resolve
    // to the same as the connected peer; expect both not to be connected for that reason.
    for (const char* addr : {"127.1", "localhost"}) {
        ASSERT_DEBUG_LOG(strprintf("Not opening a connection to %s, already connected to 127.0.0.1:8333", addr));
        BOOST_CHECK(!connman->ConnectPeer(*peerman, addr, ConnectionType::MANUAL));
    }
    ...
    BOOST_TEST_MESSAGE("\nAddNode 3 peers with IPs equivalent to existing addnode peer; expect none to be added");

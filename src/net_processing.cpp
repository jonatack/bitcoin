...
/** How many microseconds to delay requesting transactions from overloaded peers (>= MAX_PEER_TX_IN_FLIGHT requests). */
...
void RequestTx(NodeId nodeid, const GenTxid& gtxid, std::chrono::microseconds current_time) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    // Requests from txid peer while we have wtxid peers are delayed by TXID_RELAY_DELAY.
    if (!state->m_wtxid_relay && g_wtxid_relay_peers > 0) delay += TXID_RELAY_DELAY;
    // Requests from overfull peers are delayed by OVERLOADED_PEER_TX_DELAY.
...
void PeerManager::BlockConnected(const std::shared_ptr<const CBlock>& pblock, const CBlockIndex* pindex)
{
    ...
    }
    {
        LOCK(cs_main);
        for (const auto& ptx : pblock->vtx) {
            g_txrequest.AlreadyHaveTx(GenTxid{false, ptx->GetHash()});
            g_txrequest.AlreadyHaveTx(GenTxid{true, ptx->GetWitnessHash()});
        }
    }
}
                                         ...
                                         const std::atomic<bool>& interruptMsgProc)
{
        ...
        for (CInv& inv : vInv) {
            ...
            if (inv.IsMsgBlk()) {
                ...
                if (fBlocksOnly) {
                    ...
                    RequestTx(pfrom.GetId(), gtxid, current_time);
                }
            } else {
    ...
    if (msg_type == NetMsgType::TX) {
        ...
        for (const GenTxid& gtxid : {GenTxid(false, txid), GenTxid(true, wtxid)}) {
            g_txrequest.ReceivedResponse(pfrom.GetId(), gtxid);
        }
        ...
        if (!AlreadyHaveTx(GenTxid(/* is_wtxid=*/true, wtxid), m_mempool) &&
            ...
            m_mempool.check(&::ChainstateActive().CoinsTip());
            for (const GenTxid& gtxid : {GenTxid(false, txid), GenTxid(true, wtxid)}) {
                g_txrequest.AlreadyHaveTx(gtxid);
            }
        ...
        else if (state.GetResult() == TxValidationResult::TX_MISSING_INPUTS)
        {
            ...
            if (!fRejectedParents) {
                ...
                for (const uint256& parent_txid : unique_parents) {
                    ...
                    if (!AlreadyHaveTx(gtxid, m_mempool)) RequestTx(pfrom.GetId(), gtxid, current_time);
                }
                AddOrphanTx(ptx, pfrom.GetId());

                // Once added to the orphan pool, a tx is considered AlreadyHave, and we shouldn't request it anymore.
                g_txrequest.AlreadyHaveTx(GenTxid{false, tx.GetHash()});
                g_txrequest.AlreadyHaveTx(GenTxid{true, tx.GetWitnessHash()});

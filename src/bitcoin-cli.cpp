...
private:
    ...
    std::string ConnectionTypeForNetinfo(const std::string& conn_type, bool is_block_relay) const
    {
        if (conn_type == "inbound") return is_block_relay ? "block" : "full";
...
public:
    ...
    UniValue ProcessReply(const UniValue& batch_in) override
    {
        ...
        for (const UniValue& peer : batch[ID_PEERINFO]["result"].getValues()) {
            ...
            if ((is_block_relay && !is_outbound) || conn_type == "block-relay-only") { // check if {inbound, outbound}-block-relay
                ++m_counts.at(is_outbound).at(m_networks_size + 1); // in/out block-relay
                ++m_counts.at(2).at(m_networks_size + 1);           // total block-relay
            }
            if (conn_type == "manual") {
                ++m_counts.at(is_outbound).at(m_networks_size + 2); // in/out manual
            }
            if (DetailsRequested()) {
        ...
        if (DetailsRequested() && !m_peers.empty()) {
            ...
            for (const Peer& peer : m_peers) {
                ...
                result += strprintf(
                    ...
                    ConnectionTypeForNetinfo(peer.conn_type, peer.is_block_relay),
                    peer.network,

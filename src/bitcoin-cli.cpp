    ...
    UniValue ProcessReply(const UniValue& batch_in) override
    {
        ...
        for (const UniValue& peer : batch[ID_PEERINFO]["result"].getValues()) {
            ...
            if (DetailsRequested()) {
                ...
                const bool is_addr_relay_enabled{peer["addr_relay_enabled"].get_bool()};
        ...
        if (DetailsRequested() && !m_peers.empty()) {
            ...
            result += strprintf("<->   type   net  mping   ping send recv  txn  blk  hb %*s%*s%*s ", m_max_age_length, "age", m_max_addr_rate_limited_length, "addrl", m_max_addr_processed_length, "addrp");
